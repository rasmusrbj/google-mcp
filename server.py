...
if __name__ == "__main__":
    ...
    if transport == 'sse':
        ...
        # Token storage path (ephemeral on Railway, but works between requests in same session)
        TOKEN_FILE = Path("/tmp/google_token.json")

        # OAuth flow state storage
        oauth_flow = None
        ...
        async def login_handler(request):
            ...
            if not all([client_id, client_secret, redirect_uri]):
                return HTMLResponse(
                    """
                    <html><body style="background:#27272a;color:#fafafa;font-family:sans-serif;text-align:center;padding:50px;">
                    <h1>⚠️ Configuration Error</h1>
                    <p>Missing OAuth credentials in environment variables.</p>
                    <p>Please set GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, and GOOGLE_REDIRECT_URI</p>
                    </body></html>
                    """,
                    status_code=500
                )
        ...
        async def oauth_callback_handler(request):
            ...
            if not oauth_flow:
                return HTMLResponse(
                    ...
                    <p>OAuth flow not initialized. Please visit <a href="/login" style="color:#60a5fa;">/login</a> first.</p>
                    </body></html>
                    """,
                    status_code=400
                )
            ...
            try:
                ...
                return HTMLResponse(
                    ...
                    <p>Google Workspace MCP is now authenticated.</p>
                    <p>You can close this window and use the MCP server.</p>
                    <p style="margin-top:30px;font-size:14px;color:#a1a1aa;">
                    Server URL: <code style="background:#18181b;padding:5px 10px;border-radius:4px;">{}</code>
                    </p>
                    </body></html>
                ...
                return HTMLResponse(
                    ...
                    <p>{str(e)}</p>
                    <p><a href="/login" style="color:#60a5fa;">Try again</a></p>
                    </body></html>
                    """,
                    status_code=500
                )
        ...
        async def status_handler(request):
            ...
            if authenticated:
                try:
                    ...
                    email = creds_data.get('account', 'authenticated user')
                    status_text = f"✅ Authenticated as {email}"
                    status_color = "#22c55e"
                except:
                    status_text = "✅ Authenticated (email unknown)"
                    status_color = "#22c55e"
            else:
            ...
            return HTMLResponse(
                f"""
                <html><body style="background:#27272a;color:#fafafa;font-family:sans-serif;text-align:center;padding:50px;">
                <h1 style="color:{status_color};">Google Workspace MCP Server</h1>
                <p style="font-size:18px;margin:20px 0;">{status_text}</p>
                {'' if authenticated else '<p><a href="/login" style="background:#3b82f6;color:white;padding:10px 20px;border-radius:6px;text-decoration:none;display:inline-block;">Login with Google</a></p>'}
                <div style="margin-top:40px;padding:20px;background:#18181b;border-radius:8px;max-width:600px;margin-left:auto;margin-right:auto;">
                <h3 style="margin-top:0;">MCP Server Info</h3>
                <p style="font-size:14px;color:#a1a1aa;text-align:left;">
                <strong>Tools:</strong> 229 Google Workspace tools<br>
                <strong>Transport:</strong> SSE (Server-Sent Events)<br>
                <strong>Endpoint:</strong> <code>/sse</code><br>
        ...
        # Update get_credentials to use cloud token file
        original_get_credentials = get_credentials

        def get_credentials_cloud():
            """Get credentials from cloud token file or environment."""
            # Try environment variable first (for manual setup)
            ...
            # Try cloud token file (from /login flow)
            ...
            raise Exception("Not authenticated. Please visit /login to authenticate with Google.")

        # Monkey-patch get_credentials for cloud mode
        import sys
        current_module = sys.modules[__name__]
        setattr(current_module, 'get_credentials', get_credentials_cloud)

        # Create Starlette app with routes
        routes = [
            Route('/', status_handler),
            Route('/login', login_handler),
            Route('/oauth2callback', oauth_callback_handler),
        ]
        ...
        print(f"Visit http://localhost:{port}/login to authenticate")

        mcp.run(transport='sse', host='0.0.0.0', port=port, app=app)
    else:
